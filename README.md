...
- Two further charts, from separate DESNZ dataset families (plus one UK-wide DUKES figure), cover local energy rather than emissions: renewable electricity generation by technology (solar, wind, hydro, bioenergy & waste), whose "i" button opens to a figure comparing it to local electricity demand; and energy consumption, which can be split "By fuel type" (Fossil fuels and Green energy in the default "simple" view, or seven constituent categories via "Show all fuel types") or "By sector" (Domestic, Transport, Industrial/Commercial and other). DESNZ publishes a single "Electricity" consumption figure with no source attribution, so this site splits it into "Fossil fuel electricity" and "Renewable electricity" using that year's DUKES national generation mix, applied uniformly to every area (not netted against local generation — see "Known limitations" below for why). Hovering a "By fuel type" bar shows its constituent breakdown without needing to tick "Show all fuel types" first, and hovering either electricity-split row shows the calculation itself. See each chart's "i" button for what they measure, why generation and consumption aren't directly connected (Great Britain's shared national grid means locally generated power isn't routed to local homes and businesses), and why a large "Industrial, Commercial and other" share can make an area's energy consumption look disproportionately high next to its emissions (New Forest's oil refining is the clearest example). Both respond to the page-wide Totals/Per person toggle (GWh/ktoe vs kWh and toe per person) and the Latest year/Historical trend toggle.
- A "Fixed scale" axis (the default, toggled next to the region selector) shares one axis maximum across every region being compared, so switching regions doesn't rescale the chart and hide a real difference in volume. For the sector, gas, generation and consumption charts, total figures share one scale per tier (historic districts / current unitaries / proposed unitaries); per-person figures instead share one scale across all 19 regions, since per-person values already divide out the population difference that makes tiers necessary for totals. The trend chart always uses one scale across all 19 regions, on either metric, since it can show more than one tier on the same chart at once. The sector, gas and trend charts' fixed scale also spans both the 100-year and 20-year GWP time horizons at once, so switching that toggle for a single region doesn't rescale the axis either. "Auto scale" reverts to sizing each chart to its own region's figures. "Latest year" and "Historical trend" deliberately use different maxima even in fixed mode: latest scales to the highest value in that one year (so a real decline since earlier years doesn't leave every current figure looking artificially small against a much higher historical peak), historical scales to the highest value across the full published history (so its line doesn't rescale as you scan across years). Each of these five charts' "i" button names which specific region/year/category (and, for sector, gas and trend, GWP horizon) currently sets its fixed axis — worth checking whenever a chart's axis looks larger than the selected region's own figures would suggest, since the ceiling can come from a different region entirely, or from that region's *other* horizon.
...
- DESNZ publishes a single "Electricity" consumption figure with no source attribution, since the electricity consumed locally is drawn from the national grid mix (gas, nuclear, wind, solar, imports, etc.), which this dataset doesn't attribute back to the consuming area. This site fills that gap by splitting it into "Fossil fuel electricity" and "Renewable electricity" using that year's DUKES national generation mix, applied the same way to every area's own electricity consumption — deliberately *not* netted against an area's own local renewable generation. An earlier version of this feature did net local generation off first (as its own separate "Electricity: green vs fossil" chart) — that fell apart on a concrete case: New Forest hosts Marchwood, a large (~900MW) gas-fired power station invisible to that calculation, since only renewable generation is published by local authority, so crediting only local renewables (not the unmeasured local fossil generation) made New Forest look greener than the national average despite hosting a major fossil plant. Since the grid pools everything, no area's actual electricity draw can honestly differ from the national mix based on what's built nearby, so this site now applies one uniform ratio to everyone. "Green energy" (the simple view's label) combines this estimate with Bioenergy and wastes (a real, measured local quantity) — two different kinds of "green," summed under one label; see the consumption chart's "i" button.
...
- The sector/gas/generation/consumption charts' "Fixed scale" (the default) shares one axis maximum across every region in the comparison set — the highest single value seen for that chart/metric/breakdown across the set, not just what's on screen — so a smaller region's bar can end up short enough that its own internal split is hard to read. That comparison set is one tier (historic districts / current unitaries / proposed unitaries) for total figures, but all 19 regions for per-person figures, since per-person values already divide out the population difference tiers exist to separate; see `tierScopeKey`/`tierScopeRegions` in `assets/js/app.js`. "Auto scale" trades that comparability away for a clearer read of one region's own composition; see `assets/js/app.js`'s `sectorTierMax`/`gasTierMax`/`generationTierMax`/`consumptionTierMax`. Each of these takes an `onlyYear` argument: the latest-year bar view passes its dataset's own latest year (so the axis reflects what's actually achievable *now*, not diluted by an unrelated peak elsewhere in the comparison set's history); the historical trend view omits it, scanning every year, so its line doesn't rescale as you scan across years within that one view. The two views' axes are expected to differ from each other as a result — a bar chart and a line chart aren't shown side by side, so there's no need for them to share one number. `sectorTierMax` and `gasTierMax` also scan both GWP horizons at once via `withHorizon` (not just whichever is currently selected), so a region's own 100-year/20-year toggle doesn't rescale the axis either. The sector chart's latest-year bars use one symmetric scale either side of zero (`sectorTierMax`, the largest magnitude in either direction — needed since a diverging bar's left/right length has to stay comparable); its historical trend line instead uses the comparison set's actual highest and lowest values independently (`sectorTierRange`), since a line's vertical position isn't a length comparison the same way. The sector chart's sub-sector detail view is a further case that can only ever share a scale across the latest year, regardless of view, since DESNZ only publishes sub-sector figures for the latest year.
- The trend chart's "Fixed scale" is a single maximum across all 19 regions and both GWP horizons (`trendGlobalMax`), not a per-tier or per-horizon one — it can show regions from more than one tier on the same chart at once (a "context" view mixes a district, its unitary and Hampshire and the Solent), so there's no single tier to scale it to, and the point of a fixed scale is to make a region's own horizon switch legible too. This means Hampshire and the Solent (present in every context view) renders at a consistent size regardless of which region is selected, at the cost of an individual district's own line reading small next to it under "Totals" — the same comparability-vs-detail trade-off the other charts' fixed scale makes. Like the other charts, its latest-year bar view scales to the latest year's own maximum, and its historical trend view scans the full published history.
