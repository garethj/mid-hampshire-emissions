...
`refresh_data.py` doesn't cover the electricity green/fossil chart's DUKES 6.5a national grid-mix ratio — that's not part of either DESNZ collection above, so there's nothing to auto-fetch. Add each new year by hand: read rows 9 ("Fossil fuels") and 10 ("Low carbon") of that year's [DUKES table 6.5a](https://www.gov.uk/government/statistics/electricity-chapter-6-digest-of-united-kingdom-energy-statistics-dukes), then add a `{ year: { greenPct, fossilPct } }` entry to `DUKES_ELECTRICITY_MIX` in `assets/js/app.js` (no data regeneration needed — it's a small client-side lookup table, not part of the pre-processed data files).
...
- The generation/consumption charts' "Fixed scale" (the default) shares one axis maximum across every region in the same tier — the highest single value seen for that chart/metric/breakdown across every region in the tier and every year of data, not just what's on screen — so a smaller region's bar can end up short enough that its own internal split is hard to read. "Auto scale" trades that comparability away for a clearer read of one region's own composition; see `assets/js/app.js`'s `generationTierMax`/`consumptionTierMax`.
- The electricity green/fossil split is a deliberately indicative estimate, not a metered figure: no dataset ties a specific unit of electricity consumed in one area back to where it was generated. It assumes an area's own renewable generation fully offsets its own consumption first (the same "market-based" accounting logic, and the same simplification as the generation-vs-demand figure above — in reality that generation is exported to the shared grid and pooled), then applies DUKES 6.5a's national low-carbon/fossil ratio to whatever's left. It's also currently 2024-only (see "Updating the data for a new year" above) and doesn't respond to the Latest year/Historical trend toggle, since DUKES's grid-mix ratio isn't part of the sub-national datasets this site otherwise updates automatically.
